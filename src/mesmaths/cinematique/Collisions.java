...
public class Collisions {
    static final double EPSILON_CHOC_BILLE = 3.0E-2;  // en deça de cette valeur pour |u.(v1-v2)| le choc entre 2 billes est considere statique
    static final double EPSILON_CHOC_PAROI = 3.0E-2;  // en deça de cette valeur pour |J.v| le choc entre une bille et la paroi est considere statique
     ...
     * <p>
     * <p>
     * c-à-d que en entree on considère que (position,vitesse, acceleration) sont le vecteur position, le vecteur vitesse et le vecteur acceleration de la bille immediatement avant le choc
     * et en sortie (position,vitesse, acceleration) sont le vecteur position, le vecteur vitesse et le vecteur acceleration de la bille immediatement après le choc
     * <p>
     * si le choc est dynamique, le vecteur vitesse est modifie par la collision (comme une boule de billard l'est par une bande)
     * si le choc est statique, le vecteur acceleration est modifie (on considère que le choc est elastique et une force de rappel est appliquee à la bille)
     * <p>
    ...
    static boolean actionReactionBilleSegmentAvecRebond(Vecteur position, double rayon, Vecteur vitesse, Vecteur acceleration, double masse, Vecteur P0, Vecteur P1) {
        ...
        {                                                            ////System.err.println("intersection avec : "+ P0 +" et "+ P1 );
            ...
            if (vJ < -Collisions.EPSILON_CHOC_PAROI) // la bille sort du contour, le choc est dynamique
            {
                ...
                // à present calcul de la nouvelle position : qui n'est plus utilisee
                ...
                return true;
            } else if (vJ > Collisions.EPSILON_CHOC_PAROI)               // la bille revient : on n'a rien à faire
     ...
     * <p>
     * <p>
     * c-à-d que en entree on considère que (position,vitesse) sont le vecteur position et le vecteur vitesse de la bille immediatement avant le choc
     * et en sortie (position,vitesse) sont le vecteur position et le vecteur vitesse de la bille immediatement après le choc
     * <p>
     * le vecteur vitesse est modifie par la collision (comme une boule de billard l'est par une bande)
     * le vecteur position est modifie : la bille "ressort" du bord. En effet, en entree si on detecte une collision, c'etait parce que la bille avait legèrement penetre
    ...
    static boolean collisionBilleSegmentAvecRebond1(Vecteur position, double rayon, Vecteur vitesse, Vecteur P0, Vecteur P1) {
        ...
        if (t.length == 2) {                                                            ////System.err.println("intersection avec : "+ P0 +" et "+ P1 );
            Vecteur[] base = Geop.base(P0, P1);
            Vecteur I = base[0];
            Vecteur J = base[1];
     ...
     * si la collision est statique, le vecteur acceleration est modifie par la collision (une force de rappel est appliquee à la bille)
     */
     ...
     * si il y a collision, le vecteur position est modifie : la bille "ressort" sur le bord oppose au bord qu'elle penètre.
     *
    ...
    /*
     * utile à la methode collisionBilleContourPasseMuraille
     * */
    ...
    /**
     * gestion de la collision avec arrêt suivant la direction horizontale  de la bille definie par (position,rayon,vitesse) avec un contour rectangulaire de l'ecran.
     * <p>
     ...
     * <p>
     * Dès qu'il y a collision avec un bord vertical, la composante horizontale du vecteur vitesse est annulee, de sorte que la bille continue à glisser le long de
     * la bande qui l'a arrêtee
     */
...
//vitesse.x = Collisions.arretSurBord(vitesse.x, position.x, rayon, abscisseCoinHautGauche, largeur);
    ...
    /**
     * gestion de la collision avec arrêt suivant la direction verticale  de la bille definie par (position,rayon,vitesse) avec un contour rectangulaire de l'ecran.
     * <p>
     ...
     * alors vitesse est modifie et  position reste intact
     * <p>
     * Dès qu'il y a collision avec un bord horizontal, la composante verticale du vecteur vitesse est annulee, de sorte que la bille continue à glisser le long de
     * la bande qui l'a arrêtee
     */
...
//vitesse.y = Collisions.arretSurBord(vitesse.y, position.y, rayon, ordonneeCoinHautGauche, hauteur);
    ...
    /**
     * gère la collision des 2 billes definies respectivement par
     ...
     * Les nouvelles positions et nouveaux vecteurs vitesses sont calcules pour representer l'etat des billes immediatement après le choc
     */
...
// resultats : b1 et b2 après le choc
    {
        ...
        else                // il y a collision entre les 2 billes
        {
            ...
            // à present calcul des positions après le choc
...
//nG1G2 = G1G2.norme();
    ...
    /**
     * gère la collision dynamique ou statique des 2 billes definies respectivement par
     ...
     * <p>
     * Les nouvelles accelerations ou les nouveaux vecteurs vitesses sont calcules pour representer l'etat des billes immediatement après le choc
     */
...
//nG1G2 = G1G2.norme();
        ...
        if (nG1G2 >= r || nG1G2 < Collisions.EPSILON)     // il n'y a pas de collision entre les 2 billes
            ...
            if (a < Collisions.EPSILON_CHOC_BILLE) {
                if (a > -Collisions.EPSILON_CHOC_BILLE) {                                           // choc sans vitesse
            ...
            } else      /*  a >= 0 */          // il y a choc elastique entre les 2 billes
            {
                ...
                // à present calcul des positions après le choc qui n'est plus utilisee
